"""Chaos flag contract for certification-service.

The external chaos-service sets Redis string keys (with TTLs). This module runs
...
Keys polled:
    chaos:latency:certification-service  -> int ms of latency injected per request
    chaos:error:certification-service    -> int 0-100 percent of requests to fail
    chaos:cpu:certification-service      -> int busy-loop worker thread count
    chaos:memleak:certification-service  -> int MB/sec leaked while the flag is set
"""
...
logger = structlog.get_logger('certification-service.chaos')
...
ALL_KEYS = [LATENCY_KEY, ERROR_KEY, CPU_KEY, MEMLEAK_KEY]
...
class ChaosPoller:
    ...
    async def _poll_once(self) -> None:
        ...
        latency_raw, error_raw, cpu_raw, memleak_raw = values
