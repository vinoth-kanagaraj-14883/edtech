...
// Local JSON logger consistent with search-service's index.ts logging style.
const log = (level: string, msg: string, extra: Record<string, unknown> = {}): void => {
  console.log(JSON.stringify({ level, service: SERVICE_NAME, timestamp: new Date().toISOString(), msg, ...extra }));
};

// Dedicated Redis client for chaos flag polling. A separate connection avoids
// coupling chaos polling to the service's primary Redis usage. Redis is a soft
// dependency — the service must keep working when it is unavailable.
...
const syncCpuWorkers = (target: number): void => {
  ...
  log('info', 'chaos cpu workers updated', { workers: target });
};
...
const memleakTick = (): void => {
  if (flags.memleakMbPerSec > 0) {
    try {
      ...
      log('warn', 'chaos memleak allocation failed', {
        error: error instanceof Error ? error.message : String(error)
      });
    }
  } else if (leakedBuffers.length > 0) {
    leakedBuffers.length = 0;
    log('info', 'chaos memleak released retained buffers');
  }
};
...
export const chaosMiddleware = async (
  ...
  if (latencyMs > 0) {
    trace.getActiveSpan()?.setAttribute('chaos.injected', true);
