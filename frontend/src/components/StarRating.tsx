...
export default function StarRating({ value, count, size = 'sm', onDark = false, className }: StarRatingProps) {
    ...
    <div className="flex" aria-hidden="true">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg
          ...
          height={starPx}
          viewBox="0 0 20 20"
          className={filled ? 'text-star' : onDark ? 'text-white/25' : 'text-ink-300 dark:text-ink-600'}
          fill="currentColor"
        >
  ...
  const valueColor = onDark ? 'text-star' : 'text-warning-600';
  const countColor = onDark ? 'text-white/70' : 'text-content-subtle';
